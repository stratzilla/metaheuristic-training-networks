...
filenames = [i for i in glob.glob(f'{argv[1]}/*.csv')]
...
comb.columns = [i for i in range(len(comb.columns))]
# get the 20 worst performing runs
worst_columns = (comb.iloc[-1].nlargest(20).index)
# and remove them from the df
comb = comb.drop(worst_columns, axis=1)
# now set permanent name
